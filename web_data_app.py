...
def book_data():
    ...
    # Calculate the sale prices (25% reduction)
    sale_prices = [price * 0.75 for price in prices]

    # Create a DataFrame using the three lists
    book_data = pd.DataFrame(list(zip(titles, prices, sale_prices)), columns=['Titles', 'Original Prices', 'Sale Prices'])
...
def learn():
    """
    This route returns a string describing one thing learned in ENSF 692.
    """
    return "In ENSF 692, I have learned how to build web applications using Flask and perform web scraping with BeautifulSoup."
